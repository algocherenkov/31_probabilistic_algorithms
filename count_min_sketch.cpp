...
int32_t cms_check_mean_min_alt(CountMinSketch* cms, uint64_t* hashes, int num_hashes) {
    ...
    int32_t i, num_add = 0;
    int64_t* mean_min_values = static_cast<int64_t * >(calloc(cms->depth, sizeof(long)));
    for (i = 0; i < cms->depth; ++i) {
        uint32_t bin = (hashes[i] % cms->width) + (i * cms->width);
...
static int __setup_cms(CountMinSketch* cms, unsigned int width, unsigned int depth, double error_rate, double confidence, cms_hash_function hash_function) {
    ...
    cms->elements_added = 0;
    cms->bins = static_cast<int32_t*>(calloc(width * depth, sizeof(int32_t)));
    cms->hash_function = (hash_function == NULL) ? __default_hash : hash_function;
...
static void __read_from_file(CountMinSketch* cms, FILE *fp, short on_disk, const char* filename) {
    ...
    if (on_disk == 0) {
        cms->bins = static_cast<int32_t*>(malloc(length * sizeof(int32_t)));
...
static uint64_t* __default_hash(int num_hashes, const char* str) {
    uint64_t *results = static_cast<uint64_t*>(calloc(num_hashes, sizeof(uint64_t)));
    int i;
